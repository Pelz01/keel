...
async function main() {
  ...
  if (chainId !== 1952) {
    throw new Error(`Refusing to run this testnet validation on chain ${chainId}`);
  }
  ...
  const requiredUsdg = hre.ethers.parseUnits("0.25", usdgDecimals);
  const requiredOkb = hre.ethers.parseEther("0.2");
  if (usdgBefore < requiredUsdg) throw new Error("Not enough testnet USDG for validation");
  if (nativeBefore < requiredOkb) throw new Error("Not enough testnet OKB for validation");

  const PoolManager = await hre.ethers.getContractFactory("TestnetPoolManager");
  ...
  const executor = await SwapExecutor.deploy(poolManagerAddr);
  ...
  const initializeReceipt = await wait(await executor.initialize(poolKey, sqrtPriceX96));
  ...
  const position = Position.fromAmounts({
    ...
    tickLower,
    tickUpper,
    amount0: hre.ethers.parseEther("0.1").toString(),
    amount1: hre.ethers.parseUnits("10", usdgDecimals).toString(),
  ...
  await wait(await usdg.approve(executorAddr, hre.ethers.MaxUint256));
  ...
  const neutralSwapAmount = hre.ethers.parseEther("0.005");
  const toxicSwapAmount = hre.ethers.parseEther("0.003");
  const healingSwapAmount = hre.ethers.parseUnits("0.005", usdgDecimals);
